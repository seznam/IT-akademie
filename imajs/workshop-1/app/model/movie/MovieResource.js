...
export default class MovieResource extends AbstractResource {
	...
	getMovies(query = null) {
		...
		return this._httpAgent.get(url, {}).then((response) => {
			...
			return this._entityFactory.createEntities(data.filter(
				movieData => matchesQuery(movieData)
			));
		});
	...
	getMovie(url) {
		...
		filtered <- movies where movie.url === url
		return filtered.empty ? null : filtered[0]
		 */
	}
}
